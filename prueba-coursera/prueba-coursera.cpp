...
void menuUsuario() {
    ...
    cout << "4. Guardar Usuario en archivo" << endl;
    cout << "5. Cargar Usuarios desde archivo" << endl;
    cout << "6. Eliminar Cuenta Usuario" << endl;
    cout << "7. Salir" << endl;
...
void compilaUsuario() {
    ...
    do {
        ...
        if (opcionUsuario == 1) {
            ...
            cout << "Es Premium? (s/n): "; cin >> premiumChar; cin.ignore();

            if (premiumChar == 's' || premiumChar == 'S') { premium = true; }

            Cliente<string> nuevoCliente(nombre, profesion, correo, contra, telefono, premium);
            Usuario<string> nuevoUsuario(nuevoCliente);
            ...
            if (archivo.is_open()) {
                ...
                while (getline(archivo, linea)) {
                    Cliente<string> cliente = Cliente<string>::cargarDesdeLinea(linea);
                    string correo = cliente.get_correo();

                    if (!listaUsuario.existeCorreo(correo)) {
                        Usuario<string> usuario(cliente);
            ...
            getline(cin, correo);
            listaUsuario.eliminarPorCorreo(correo);
        }
